...
History:
  ...
  2024/08/08: 0.0.11 bugfixes; yara
  2024/08/14: added --excludefilenames
...
def ProcessBinaryFile(filename, content, cutexpression, flag, oRules, dResults, oOutput, oLogfile, options, oParserFlag):
    ...
    try:
        ...
        if oRules != None:
            metadata = False
            for result in rules.match(data=data):
                if not metadata:
                    sha256 = hashlib.sha256(data).hexdigest()
                    ctime = FormatTimeUTC(os.path.getctime(filename))
                    mtime = FormatTimeUTC(os.path.getmtime(filename))
                    atime = FormatTimeUTC(os.path.getatime(filename))
                    metadata = True
                row = [filename, result.namespace, result.rule, sha256, ctime, mtime, atime]
